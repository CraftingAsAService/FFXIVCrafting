<td class='slot slot-{{ $slot->name }}{{ isset($original_level) && $original_level > $level ? ' hidden' : '' }}' data-level='{{ $level }}'>
	<div class='slot-wrap'>
		<div class='items'>
			...
			<div class='item cf {{ $key > 0 ? 'hidden' : 'active' }}{{ $item->crafted_by ? ' craftable' : '' }}' data-item-id='{{ $item->id }}' data-item-ilvl='{{ $item->ilvl }}'>
				...
				<div class='icons pull-left text-center'>
					<a href='http://xivdb.com/{{{ $item->href }}}' target='_blank'>
						<img src='/img/items/{{ $item->icon ?: '../noitemicon.png' }}' width='40' height='40' class='main-icon'>
					</a>
					<div>
						@if($item->crafted_by)
						...
						@if($item->vendors)
						<div class='gil'>
							<img src='/img/coin.png' class='stat-vendors' rel='tooltip'  width='24' height='24' title='Available from {{ $item->vendors }} vendor{{ $item->vendors != 1 ? 's' : '' }} for {{ number_format($item->gil) }} gil'>
						</div>
						@endif
					</div>
				</div>
