...
// php artisan db:seed --class=GarlandSeeder
// and as needed
...
class GarlandSeeder extends Seeder
{
	...
	private function local()
	{
		...
		else
		{
			...
			$core = $this->getCleanedJson(base_path() . '/../garlandtools/db/data/en/core/data.json');
	...
	private function node_bonuses($node_bonuses)
	{
		...
		foreach ($node_bonuses as $nb)
		{
			...
			$this->setData('node_bonuses', (array) $nb);
		}
	...
	private function fishing()
	{
		...
		foreach (array_diff(scandir(base_path() . '/../garlandtools/db/data/en/fishing'), ['.', '..']) as $file)
		{
			...
			$f = $this->getCleanedJson($json_file);
			$f = $f->fishing;
	...
	private function mob()
	{
		...
		foreach (array_diff(scandir(base_path() . '/../garlandtools/db/data/en/mob'), ['.', '..']) as $file)
		{
			...
			$m = $this->getCleanedJson($json_file);
			$m = $m->mob;
	...
	{
		echo '@' . $this->humanReadable(memory_get_usage()) . PHP_EOL;
	}
